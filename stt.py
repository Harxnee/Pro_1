...
class TextTab(QWidget):
    def __init__(self):
        super().__init__()
        acceptButton = QPushButton('Convert', self)
...
class VoiceTab(QWidget):
    ...
    def __init__(self):
        super().__init__()
        acceptButton = QPushButton('Convert', self)
        acceptButton.setGeometry(QRect(320, 100, 85, 27))  
        acceptButton.clicked.connect(self.callSpeechToText)

        searchButton = QPushButton('Find', self)
        searchButton.setGeometry(QRect(10, 60, 85, 27))
        searchButton.clicked.connect(self.openFileExplorer)
